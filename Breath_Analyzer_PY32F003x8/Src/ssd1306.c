...
void SSD1306_DrawRoundedRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, SSD1306_COLOR_t c)
{
	...
	/* Draw 4 lines */
	SSD1306_DrawLine(x + r, y, x + w - r, y, c);         // Top line
	SSD1306_DrawLine(x + r, y + h, x + w - r, y + h, c); // Bottom line
	SSD1306_DrawLine(x, y + r, x, y + h - r, c);         // Left line
	SSD1306_DrawLine(x + w, y + r, x + w, y + h - r, c); // Right line

	/* Draw Corners */
	SSD1306_DrawArc(x + w - r, y + r, r, 1, c);			// First quadrant
	SSD1306_DrawArc(x + r, y + r, r, 2, c);				// Second quadrant
	SSD1306_DrawArc(x + r, y + h - r, r, 3, c);			// Third quadrant
...
void SSD1306_DrawArc(int16_t x0, int16_t y0, int16_t r, int16_t quad, SSD1306_COLOR_t c)
{
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
